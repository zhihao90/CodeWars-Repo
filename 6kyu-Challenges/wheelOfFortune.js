...
function winner(candidates) {
    ...
    let bestScore = candidates[0].scores.reduce((a,b) => a +b, 0)
    let bestCand = candidates[0].name

    for (let i = 0; i < candidates.length; i++){
      
      if(candidates[i].name == undefined || candidates[i].scores == undefined || candidates[i].scores.length < 1 || candidates[i].scores.length > 2){
        return false;
      }
      
      for(let j = 0 ; j < candidates[i].scores.length; j++){
        if(candidates[i].scores[j] % 5 != 0 || candidates[i].scores[j] > 100 || candidates[i].scores[j] < 0){
          return false;
        }
      }
      
      let score = candidates[i].scores.reduce((a,b) => a +b, 0)
      
            
       if (score > bestScore && score <= 100){
        bestScore = score
        bestCand = candidates[i].name
       }
     }
