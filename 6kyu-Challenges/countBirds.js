...
function createReport(name){
    ...
    for (let i = 0; i < name.length; i++){
        if (name[i].includes("Labrador Duck")){
            return "Disqualified data"
        }
        let item = name[i].split(" ")
        let amount = item[item.length-1]    
        item.pop()
        let item2 = []
        ...
        if (item2.length == 1){
            ...
            code.push(item2[0].split("").slice(0,6).join("").toUpperCase())

        }else if(item2.length == 2){
            
            for(let word of item2){
                code.push(word.split("").slice(0,3).join("").toUpperCase())
            }

        }else if(item2.length == 3){

            for(let word of item2){
                code.push(word.split("").slice(0,2).join("").toUpperCase())
            }
            
        }else{

            code.push(item2[0].charAt(0).toUpperCase())
            code.push(item2[1].charAt(0).toUpperCase())
            code.push(item2[item2.length-2].split("").slice(0,2).join("").toUpperCase())
            code.push(item2[item2.length-1].split("").slice(0,2).join("").toUpperCase())

        }

        if (result.includes(code.join(""))) {

            result[result.indexOf(code.join(""))+1] = Number(result[result.indexOf(code.join(""))+1]) + Number(amount)
            
        }   else{

            result.push(code.join(""))
            result.push(Number(amount))
